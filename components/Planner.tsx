...
type OptAction =
  ...
  | {
      type: "updateColab";
      id: number;
      patch: Partial<Pick<Colaborador, "nombre" | "rol" | "modalidad">>;
    }
    ...
    case "setShift":
      return {
        ...
        schedule: {
          ...state.schedule,
          [action.fecha]: {
            ...(state.schedule[action.fecha] ?? {}),
            [action.id]: action.turno,
          },
        },
      };
    case "addColab":
...
export default function Planner({
  ...
  // El servidor (Postgres) es la fuente de verdad: el estado base se deriva de
  // los props y useOptimistic aplica los cambios al instante. Cuando la acción
  // termina, revalidatePath refresca los props y el estado se reconcilia con la
  // BD; si la escritura falla, el cambio optimista se revierte solo.
  ...
  // ---- Mutaciones (optimistas + persistencia; se reconcilian con la BD) ----
  function cycleCell(fecha: string, id: number) {
    const next = nextShift(schedule[fecha]?.[id] ?? null);
    run({ type: "setShift", id, fecha, turno: next }, () =>
      actions.setShift(id, fecha, next),
    );
  }
  ...
  function handleUpdate(
    ...
    run({ type: "updateColab", id, patch }, () =>
      actions.updateCollaborator(id, patch),
    );
  }
  ...
  const getShift = (k: string, id: number): Turno | null =>
    schedule[k]?.[id] ?? null;
  ...
  const saturdays = sats.map((s) => {
    ...
    const hasWarn = warnAM || warnPM || warnAcc || warnPresAM || warnPresPM;
  ...
  if (!anyAssigned) {
    bannerText =
      "Mes sin planificar — usa “Sugerir planificación” para una propuesta automática que puedes editar.";
    bannerColor = "#475569";
    bannerBg = "#f1f5f9";
    bannerBorder = "#e2e8f0";
  } else if (monthWarn === 0) {
        ...
        <div style={{ padding: "22px 24px 40px" }}>
          ...
          >
            ...
            <span style={{ color: "#94a3b8" }}>
              Clic en una celda para cambiar el turno
            </span>
          </div>
                ...
                <tr style={{ background: "#f8fafc" }}>
                  ...
                  {saturdays.map((s) => (
                    ...
                    >
                      ...
                      >
                        <span style={{ fontWeight: 700, color: "#334155", fontSize: 12.5 }}>
                          {s.label}
                        </span>
                        {s.isQ && (
                          <span
                    ...
                    {row.cells.map((cell) => (
                      ...
                      >
                        <button
                          type="button"
                          title={cell.full}
                          onClick={() => cycleCell(cell.fecha, row.id)}
                          style={{
                            width: "100%",
                            minWidth: 58,
                            height: 36,
          ...
          >
            Reglas aplicadas: cada turno (mañana y tarde) debe tener al menos un
            vendedor y al menos un colaborador presencial · cada colaborador tiene
            un sábado libre al mes · en los sábados de quincena (más cercano al 15 y
            al 30) se prioriza tener contabilidad en turno.
          </div>
        </div>
      )}
        ...
        <div style={{ padding: "22px 24px 40px", maxWidth: 840 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
            {roleCounts.map((rc) => (
              ...
              >
                <span
                  style={{ width: 9, height: 9, borderRadius: "50%", background: rc.dot }}
                />
                {rc.role}
                  ...
                  <tr key={c.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "7px 16px" }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                        ...
                        <select
                          value={c.rol}
                          onChange={(e) =>
                            handleUpdate(c.id, { rol: e.target.value as Rol })
                          }
                          style={teamSelect}
                        >
                          {ROLES.map((r) => (
                    ...
                    <td style={{ padding: "7px 16px" }}>
                      <select
                        value={c.modalidad}
                        onChange={(e) =>
                          handleUpdate(c.id, { modalidad: e.target.value as Modalidad })
                        }
                        style={teamSelect}
                      >
                        {MODALIDADES.map((m) => (
          ...
          >
            ...
            <select
              value={newRole}
              onChange={(e) => setNewRole(e.target.value as Rol)}
              style={teamSelect}
            >
              {ROLES.map((r) => (
                <option key={r} value={r}>
            ...
            <select
              value={newMod}
              onChange={(e) => setNewMod(e.target.value as Modalidad)}
              style={teamSelect}
            >
              {MODALIDADES.map((m) => (
                <option key={m} value={m}>
